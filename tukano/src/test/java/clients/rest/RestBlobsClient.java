...
public class RestBlobsClient extends RestClient implements Blobs {
	...
	private Result<Void> _upload(String blobURL, byte[] bytes, String token) {
		return super.toJavaResult(client.target(blobURL).queryParam(RestBlobs.TOKEN, token).request()
	...
	private Result<byte[]> _download(String blobURL, String token) {
		return super.toJavaResult(client.target(blobURL).queryParam(RestBlobs.TOKEN, token).request()
				.accept(MediaType.APPLICATION_OCTET_STREAM_TYPE).get(), byte[].class);
	}

	private Result<Void> _delete(String blobURL, String token) {
		return super.toJavaResult(client.target(blobURL).queryParam(RestBlobs.TOKEN, token).request().delete());
	}

	private Result<Void> _deleteAllBlobs(String userId, String token) {
		return super.toJavaResult(
				target.path(userId).path(RestBlobs.BLOBS).queryParam(RestBlobs.TOKEN, token).request().delete());
	}
